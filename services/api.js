...
export async function fetchStockData(ticker) {
  try {
    ...
    const ratiosRes = await fetch(`${BASE_URL_FMP}/ratios-ttm/${ticker}?apikey=${API_KEY}`);
    const ratiosData = await ratiosRes.json();

    // EPS növekedés (külön végpontból)
    const growthRes = await fetch(`${BASE_URL_FMP}/financial-growth/${ticker}?apikey=${API_KEY}`);
    ...
    if (annualIncome.length >= 2) {
      const revenues = annualIncome.map((r) => ({ year: parseInt(r.calendarYear), value: r.revenue || 0 }));
      for (let i = 0; i < revenues.length - 1; i++) {
        ...
        revenueGrowthByYear.push({ year: current.year, growthPercent: (growth * 100).toFixed(2) + "%" });
      }
    }
    const isRevenueGrowing10Percent = revenueGrowthRates.length > 0 ? revenueGrowthRates.every((g) => g >= 0.1) : "n.a.";
    ...
    // ROE (TTM alapján)
    const roe5Y = metricsData[0]?.roeTTM != null ? (metricsData[0].roeTTM * 100).toFixed(2) + "%" : "n.a.";

    // EPS növekedés (financial-growth végpontból)
    const profitGrowthRaw = growthData.length > 0 && typeof growthData[0].epsgrowth === "number" ? growthData[0].epsgrowth : null;
    const profitGrowth = typeof profitGrowthRaw === "number" ? profitGrowthRaw : "n.a.";
    ...
    const volume50Days = histData.slice(0, 50).map((day) => day.volume).filter((v) => typeof v === "number");
    const avgVolume50 = volume50Days.length > 0 ? volume50Days.reduce((sum, v) => sum + v, 0) / volume50Days.length : "n.a.";
